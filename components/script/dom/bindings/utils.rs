        ...
        if let Some((native, name, nargs)) = constructor {
            ...
            for ctor in named_constructors.iter() {
                if let Some((cnative, cname, cnargs)) = *ctor {
                    ...
                    let constructor = create_constructor(cx, cnative, cnargs, cs.as_ptr());
                    define_constructor(cx, receiver, cs.as_ptr(), constructor);
                }
            }
            interface
        }
...
fn create_constructor(cx: *mut JSContext,
                      ...
                      ctor_nargs: u32,
                      name: *const libc::c_char) -> HandleObject {
    unsafe {
...
fn create_interface_object(cx: *mut JSContext,
                           ...
                           name: *const libc::c_char) {
    unsafe {
        let constructor = create_constructor(cx, constructor_native, ctor_nargs, name);
        ...
        define_constructor(cx, receiver, name, constructor);
